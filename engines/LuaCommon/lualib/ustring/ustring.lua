...
local function internalChar( t, s, e )
	...
	for i = s or 1, e or #t do
		...
		if type( v ) ~= 'number' then
			error( string.format( "bad argument #%d to 'char' (number expected, got %s)", i, type( v ) ), 2 )
		end
		v = math.floor( v )
		if v < 0 or v > 0x10ffff then
			error( string.format( "bad argument #%d to 'char' (value out of range)", i ), 2 )
		elseif v < 0x80 then
			ret[#ret + 1] = v
		elseif v < 0x800 then
...
function ustring.gsub( s, pattern, repl, n )
	...
	elseif type( repl ) == 'string' then
		...
		error( 'Invalid argument type ' .. type( repl ) .. ' for repl', 2 )
	end
